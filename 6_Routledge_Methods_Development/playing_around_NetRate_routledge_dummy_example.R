...
library(igraph); library(visNetwork); library(truncdist)
...
for(i in 1:nrow(likelihood_mat)){
  for(j in 1:ncol(likelihood_mat)){
    likelihood_mat[i,j] <- survival(A[i, j], time_diff_mat[i, j]) * hazard(A[i, j], time_diff_mat[i, j])
  }
}
